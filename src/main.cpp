...
#if defined(NDEBUG)
# error "Bencoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd6d7aae47d3423e8e7495db51159d19241dc9a90e779198b22950b1162032ff9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bencoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bencoin Signed Message:\n";
...
{
    // Bencoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Bencoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Bencoin: 1 days
static const int64 nTargetSpacing = 2.5 * 60; // Bencoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bencoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bencoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x04ea19ca468852f3a5d6260ec0f52f473d5f2c8d0bdfef1f1107b18640383fd2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "12/05/2021 ^ HumaCoin is created and killed.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0425287c10ccf9f60bda4ca92a444982d72726cbd735311037000bd04348d7e5354d71afeff089d988fb7abfbf2cea903e17559a17c8b23280a87ecfeb18010943") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1620816973;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086413112;

        if (fTestNet)
        {
            block.nTime    = 1620816958;
            block.nNonce   = 387145645;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xa331fe409fc9bff3e367ea8a27e72066bff4c4d98ab01957f9197fc1fd559231"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc4, 0xb2, 0xde }; // Bencoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BencoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BencoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BencoinMiner(CWallet *pwallet)
{
    printf("BencoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bencoin-miner");
    ...
    try { loop {
        ...
        printf("Running BencoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BencoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BencoinMiner, pwallet));
}
